...
- **Live viewer in your browser** at ~34 fps: click to tap, drag to swipe, type on your keyboard, save screenshots. One-click **Unlock** (types your passcode from `.env`) and **Restart link** (the fix after a replug). Buttons for the system gestures that are easy to get wrong by hand (Back, Spotlight, scroll), numbered chips that walk you to any Home Screen page and say which one you are on, and a **Read thread** button that pulls the last messages of a conversation back into the page. Enter sends from the message and paste boxes, Shift+Enter or Ctrl+Enter breaks the line, and the same holds when you type straight into the phone: Enter taps Send when a Send button is on screen (a Return anywhere else), Shift+Enter and Ctrl+Enter are always a Return. **Ctrl+V with a screenshot on your PC clipboard** (Win+Shift+S) puts it on the phone's clipboard, downscaled; long-press a message field on the phone and tap Paste, or type a contact under **Text someone** and click **Send image** (the message box is the caption). **Copy phone image** does the reverse: an image copied on the phone lands on your PC clipboard. Hotkeys: **Ctrl+Shift+S** copies the screen to your clipboard as a PNG (works even with the link down), **Ctrl+Shift+H** is Home and **Ctrl+Shift+B** is Back. Swipes and wheel flicks draw the line that was sent, and a picture that stops moving because the phone stopped answering is dimmed and says so instead of pretending to be live. Works even before touch input is set up.
- **The app switcher, without the swipe.** **Open apps** lists what is open right now, newest first, read over the cable (`ios ps` joined against the installed apps). Click one to bring it up, ✕ to force-quit it. Agents get the same as `open_apps()` and `close_app("Safari")`. The real switcher screen cannot be opened from a PC: WebDriverAgent's touches never reach the home-indicator zone, measured, so this is the list instead of the picture.
...
- **Shell verbs through [declick](https://github.com/ucsandman/declick).** `declick add "mcp:python C:/path/to/sidetap/scripts/phone_mcp.py" --name iphone` turns the same MCP server into a CLI: `declick run iphone ocr --rows screen --fields text,x,y --limit 30`, `declick run iphone tap-text --text General`. The tool catalog stops riding in every session's prompt, reads come back trimmed to the fields you asked for, screenshot bytes never enter the model's context, and a subagent with no MCP tools can drive the phone from Bash. `declick daemon start` keeps the server warm (about 0.6s a call, measured, against 2 to 4s cold). The safety gates are unchanged.
...
| `send_image("Mom", r"C:\shots.png", text="look")` | send a PNG/JPEG from this PC as an attachment, optional caption: phone clipboard, Paste, Send; same approval gate |
| `save_clipboard_image(r"C:\shots\in.png")` | save the image copied on the phone to a PNG on this PC |
